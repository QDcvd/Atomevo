...
class obabel
{
    ...
    public function run()
    {
        ...
        $extension = explode(',',$get['extension']);
        ...
        if (!empty($check_tasks)) {
            ...
            switch ($check_tasks['state']) {
                ...
                case '1':
                    ...
                    $msg_param['data']['use'] = 'obabel';
                    $msg_param['data']['msg'] = 'obabel 有相同作用任务已完成';
        ...
        foreach($upload_file as $f){
            $f_arr = explode('.',$f);//文件名与后缀分离
            foreach($extension as $k => $v){
                $handle_child[] = "/root/anaconda2/envs/pliptool/bin/obabel -i".$f_arr[1]." $f -o{$v} -O ".$f_arr[0].".".$v." $operation ; ";//进行文件类型转换
            }
        }
        ...
        $msg_param['data']['resultCode'] = 1;
        $msg_param['data']['use'] = 'obabel';
        $msg_param['data']['msg'] = 'obabel 任务完成。';
