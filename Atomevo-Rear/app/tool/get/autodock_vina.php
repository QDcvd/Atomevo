...
class autodock_vina
{
    ...
    public function run()
    {
        ...
        $center = $get['center'];
        $size = $get['size'];
        $energy_range = trim($get['energy_range']);
        $exhaustiveness = trim($get['exhaustiveness']);
        $num_modes = trim($get['num_modes']);
        ...
        $conf[] = 'size_y = '.$size[1];
        $conf[] = 'size_z = '.$size[2];
        $conf[] = 'energy_range = '.$energy_range;
        $conf[] = 'exhaustiveness = '.$exhaustiveness;
        $conf[] = 'num_modes = '.$num_modes;
        ...
        $msg_param['data']['resultCode'] = 1;
        $msg_param['data']['use'] = 'autodock_vina';
        $msg_param['data']['msg'] = 'autodock_vina 任务完成。';
        $msg_param['data']['tasks_id'] = $tasks_id;
        $msg_param = json_encode($msg_param);

        //开启子进程开启句柄
        $handle[] = '/usr/bin/nohup';
        // $handle[] = './child/autodock_vina_child.php';//vina 运算 加 统计分析
        $handle[] = './child/autodock_vina_xscoreV2.php';//vina 运算 加 xscore统计分析
        $handle[] = '-r';
        $handle[] = '"' . $handle_child . '"';
        $handle[] = '-o';
        $handle[] = '"' . $out_name . '"';
