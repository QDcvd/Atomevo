...
class xscore
{
    ...
    public function run()
    {
        ...
        empty($file_name['cofactor.mol']) ? $file_name['cofactor.mol'] = '' : '';
        //子进程需要执行的句柄
        $handle_child[] = "cd $out_path ;";
        $handle_child[] = "/usr/local/bin/xscore -fixmol2 ".$file_name['ligand_mol']." ligand_for_xscore.mol2 ;";//将对接后的配体文件转换为xscore格式
        $handle_child[] = "/usr/local/bin/xscore -fixpdb ".$file_name['receptor']." receptor_for_xscore.pdb ;";//将受体文件转换为xscore格式，输入文件为蛋白质，pdb格式，加极性氢
