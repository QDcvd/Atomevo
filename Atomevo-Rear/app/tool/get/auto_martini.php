...
class auto_martini
{
     ...
     */
    public function run(){
        $param = ['id'=>'require|IsInt','mol'=>'require|IsStr','gro'=>'IsStr','xyz'=>'IsStr','verbose'=>'IsStr','fpred'=>'IsStr'];
        $get = (new validate($param, 'get'))->goCheck();
        $id = intval($get['id']);
        $mol = trim($get['mol']);
        $gro = intval($get['gro']);
        $xyz = intval($get['xyz']);
        $verbose = intval($get['verbose']);
        $fpred = intval($get['fpred']);
        ...
        //对传入字符串进行检查
        if(check_cmd($mol)){ throw new Exception("mol 字段存在非法字符");die; }
        //匹配文件ID
        $file = $this->Db->get('upload','*',['id'=>$id,'uid'=>$uid,'use'=>'auto_martini']);
        if(!$file){ throw new Exception("文件不存在");die; }
        //检查任务唯一性
        ...
        if($check_tasks){
            ...
            switch ($check_tasks['state']) {
                case '0':
                    throw new Exception("已有相同作用的任务正在执行");die;
                    break;
                case '1':
                    ...
                    break;
                case '2':
                    throw new Exception("已有相同作用的任务正在排队");die;
                    break;
                default:
                    throw new Exception("未知错误");die;
                    break;
            }
        }
        ...
        if(!$get['gro'] && !$get['xyz']){ throw new Exception("gro或xyz必须选中一种输出方式");die; }
        ...
        if (file_exists($out_path) == false){
            ...
            if (!mkdir($out_path, 0777, true)) {
                throw new Exception("out_path Mkdir Failed");die;
            }
        }
