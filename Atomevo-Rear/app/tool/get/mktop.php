...
class mktop
{
    ...
    public function run()
    {
        ...
        if (!empty($check_tasks)) {
            ...
            switch ($check_tasks['state']) {
                ...
                case '1':
                    ...
                    $msg_param['data']['use'] = 'mktop';
                    $msg_param['data']['msg'] = 'mktop 有相同作用任务已完成';
        ...
        copy($file_path, $out_path .'input_file/'. $file['upload_name']); //复制到输入目录下
        $out_file_name = getFileName($file['upload_name']) . '.top';

        //子进程需要执行的句柄
        $handle_child[] = "cd {$out_path}output_file/ ;";
        $handle_child[] = "/data/binaryroot/mktop_2.2.1/mktop_2.2.1.pl -i '../input_file/{$file['upload_name']}' -o '{$out_file_name}' -ff amber -conect no";

        $handle_child = implode(' ', $handle_child);
        $out_name = $out_path . 'output_file/log.out';
        $msg_param['uid'] = 'uid_' . $uid;
        $msg_param['type'] = 'message';
        $msg_param['data']['resultCode'] = 1;
        $msg_param['data']['use'] = 'mktop';
        $msg_param['data']['msg'] = 'mktop 任务完成。';
        $msg_param['data']['tasks_id'] = $tasks_id;
        $msg_param = json_encode($msg_param);

        //开启子进程开启句柄
        $handle[] = '/usr/bin/nohup';
        $handle[] = './child/mktop_child.php';
        $handle[] = '-r';
        $handle[] = '"' . $handle_child . '"';
        $handle[] = '-o';
        $handle[] = '"' . $out_name . '"';
