...
/**
 * -----------------
 * 需要输入的文件是input.pdb和input.dat 处理之后输出  input-martin.pdb input.top protein.itp 三个文件
 * -----------------
 */
...
class martinize_protein
{
     ...
     * @param String p         限制输出文件
     * @param String ff        可选 elnedyn22 ,elnedyn ,elnedyn22p ,martini21 ,martini22p ,martini21p ,martini22  martini22 默认
    ...
    public function run()
    {
        $param = [
            'ids' => 'require',
            'down_name' => '',
            // 'p' => 'require|in:None,All,Backbone',
            'ff' => 'require|in:elnedyn22,elnedyn,elnedyn22p,martini21,martini22p,martini21p,martini22'
        ];
        ...
        // $ff = empty(trim($get['ff'])) ? 'martini22' : trim($get['ff']);
        $ff = trim($get['ff']);
        ...
        if (!empty($check_tasks)) {
            ...
            switch ($check_tasks['state']) {
                ...
                case '1':
                    ...
                    $msg_param['data']['use'] = 'martinize_protein';
                    $msg_param['data']['msg'] = 'martinize_protein 有相同作用任务已完成';
        ...
        $insert['uid'] = $uid;
        $insert['upload_id'] = 0;
        $insert['use'] = 'martinize_protein';
        $insert['down_name'] = $down_name;
        $tasks_id = $this->Db->insert('tasks', $insert);
        $tasks_id = $this->Db->id();

        //创建下载目录
        $out_path = '/data/wwwroot/mol/down/martinize_protein/' . $tasks_id . '/';
        ...
        foreach ($file as $k => $v) {
            ...
            $save_name = explode('.', $v['save_name']);
            $suffix = $save_name[1];
            copy($file_path, $out_path . "/input.$suffix");
        }

        //子进程需要执行的句柄

        $handle_child[] = "cd $out_path ;";
        $handle_child[] = "/usr/local/bin/dssp -i input.pdb -o input.dat ;";
        $handle_child[] = "/root/anaconda2/envs/my-rdkit-env/bin/python /data/pyroot/martinize_protein/martinize.py -f input.pdb -o input.top -x input-martin.pdb -ss input.dat -p backbone -ff $ff ;";
        ...
        $msg_param['data']['resultCode'] = 1;
        $msg_param['data']['use'] = 'martinize_protein';
        $msg_param['data']['msg'] = 'martinize_protein 任务完成。';
        $msg_param['data']['tasks_id'] = $tasks_id;
        $msg_param = json_encode($msg_param);

        //开启子进程开启句柄
        $handle[] = '/usr/bin/nohup';
        $handle[] = './child/martinize_protein_child.php';
        $handle[] = '-r';
        $handle[] = '"' . $handle_child . '"';
        $handle[] = '-o';
        $handle[] = '"' . $out_name . '"';
