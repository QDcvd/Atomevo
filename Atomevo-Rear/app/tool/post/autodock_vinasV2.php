...
//复制输入的文件
foreach($file as $v){
    $name = str_safe_trans($v);
    copy($out_path . 'input_file/'.$v, $out_path . 'output_file/'.$name);
}

$excel = new excel();
//判断是用表格参数还是使用输入参数
if (empty($c['center']) && empty($c['size'])) {
    //复制输入的文件
    // recurse_copy($out_path . 'input_file/', $out_path . 'output_file');
    ...
    foreach ($configure as $k => &$v) {
        ...
        //更新运行时间 (防止超时)
        $db->update('tasks', ['run_time' => time()], ['id' => $tasks_id]);
        ...
        $handle[] = "/data/pyroot/mgltools/pythonsh /data/pyroot/mgltools/prepare_ligand4.py  -l '{$v['ligand']}' -o '{$ligand_pdbqt}.pdbqt' -A hydrogens;";
        $handle[] = "/data/pyroot/mgltools/pythonsh /data/pyroot/mgltools/prepare_receptor4.py  -r '{$v['receptor']}' -U waters -o '{$receptor}.pdbqt' -A hydrogens ;";
        ...
        foreach ($files as $key => $file) {
            if (preg_match("/" . patt_trans($ligand_pdbqt) . "_out_ligand_(.*?).pdbqt/", $file)) {
                ...
                exec("cd $output_file_path ; /root/anaconda2/envs/pliptool/bin/babel -ipdbqt '$file' -omol2 '$mol_name' -h");
                //追加写入 ligand.mol
                ...
                //分析log并将数据存储在data数组中
                $data[$i] = readXscoreLog($output_file_path . 'xscore.log'); //获取log中的数据
                $data[$i]['vina'] = 'vina' . ($k + 1);
                $data[$i]['file'] = $file;
                $data[$i]['receptor'] = $v['receptor'];
                $data[$i]['ligand'] = $v['ligand'];
                $data[$i]['species'] = $species;
                $data[$i]['mode'] = $vina_csv[$i]['mode'];
                $data[$i]['affinity'] = $vina_csv[$i]['affinity'];
                $data[$i]['dist_from'] = $vina_csv[$i]['dist_from'];
                $data[$i]['best_mode'] = $vina_csv[$i]['best_mode'];
                $data[$i]['affinity_avg'] = $affinity_avg;
                $data[$i]['affinity_best'] = $affinity_best;
                $i++;
    ...
    //将输出的log文件提取关键内容并转换为csv表格
    $table = readVinaLog($task['out_path'] . 'vina.log');

    $csv_files[] = $output_file_path . 'vina_tab.csv';
    file_put_contents($output_file_path . 'vina_tab.csv', $table);
}
