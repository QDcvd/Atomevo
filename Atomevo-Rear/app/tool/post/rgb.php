...
class rgb
{
    ...
    public function run()
    {
        ...
        //创建目录
        $out_path = '/data/wwwroot/mol/down/rgb/' . $tasks_id . '/';
        $mk_dir[] = $out_path;
        ...
        foreach ($file as $v) {
            ...
            copy($file_path, $out_path . 'input_file/input.' . getFileExt($v['upload_name'])); //复制到目录
        }

        //子进程需要执行的句柄
        $handle_child[] = "cd {$out_path}output_file;";
        $handle_child[] = "/root/anaconda2/bin/python /data/pyroot/rgb/color-detection.py ../input_file/input.tif ../input_file/input.csv;";
        $handle_child[] = "/root/anaconda2/bin/python /data/pyroot/rgb/csvcombine.py ../input_file/input.csv ./RGB.csv;";
