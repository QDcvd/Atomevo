...
class glapd
{
    ...
    public function run()
    {
        ...
        //匹配文件ID
        $files = $this->Db->select('upload', '*', ['id' => $ids, 'uid' => $uid, 'use' => __CLASS__]);
        if (!$files) {
        ...
        //创建目录
        $out_path = '/data/wwwroot/mol/down/'.__CLASS__.'/' . $tasks_id . '/';
        $mk_dir[] = $out_path;
        ...
        $handle_child[] = "cp -r /data/binaryroot/GLAPD/Par/ {$out_path}/Par/;";//复制所需的运行文件
