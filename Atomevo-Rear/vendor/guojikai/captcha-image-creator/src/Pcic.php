...
class Pcic
{
	...
	private $fontSize = 35; //defualt font size
  	private $fontFamily = [
  		...
  		'wanghan.ttf',
  		'Hamurz Free Version.ttf',
  		'Kaushanscript.ttf',
  		'kongying.ttf',
  		'Akronim-Regular.ttf',
  		'shiti-Heavy.ttf',
  		'BarlowCondensed-ExtraLight.ttf',
  		'BalooBhaina-Regular.ttf',
  	]; //default font family
	...
	//construct
	function __construct($string, $width, $height,$fontFamily) {
		$this->fontFamily = $fontFamily ? $this->fontFamily[$fontFamily] : $this->fontFamily[0];
  ...
  //create temp image
	private function createTemp() {
		...
		imagecolorallocate($this->captchaTemp, 0, 0, 0);
	}
	...
	private function writeCode() {
		...
		for($i=0; $i<$this->getStringLength(); $i++) {
			...
			$color = imagecolorallocate($this->captchaTemp, 0, 0, 0);
  ...
  //create image
    ...
    imagecolorallocatealpha($this->captchaImage, 0, 0, 0 ,127);
    imagealphablending($this->captchaImage , false);
	 ...
	 * @return image
	 */
	public static function createCaptchaImage($string = '', $width = 0, $height = 0, $fontFamily=false)
	{
		$pcic = new self($string, $width, $height ,$fontFamily);
    	$pcic->printImage();
	}
