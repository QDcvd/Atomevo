...
foreach ($configure as $k => $v) {
    ...
    $handle[] = "/data/pyroot/mgltools/pythonsh /data/pyroot/mgltools/prepare_receptor4.py  -r '" . $v['receptor'] . "' -U waters -o '{$receptor}_replace.pdbqt' $receptor4_A ;";
    $handle = implode(' ', $handle);
    exec($handle, $out, $sat); //运行程序

    //获取转换之后文件并检查对其重新排列
    $receptor_content = file_get_contents($output_file_path.$receptor.'_replace.pdbqt');
    $receptor_content = explode("\n",$receptor_content);
    $new_content = [];
    foreach ($receptor_content as $key=>$value){
        //取出最后三个字符块
        $last = substr($value, -3);
        //取出要处理的字符
        $newstr = substr($value,0,strlen($value)-3);
        $length = strlen($newstr);
        $replace_content = '';
        if($length != 76){
           if($length > 76){
               //去掉一个空格（字符串第29和30个的位置）
               $new_1 = substr($newstr,0,29);
               $new_2 = substr($newstr,30);
               $replace_content = $new_1.$new_2;

           }else{
               //加上一个的空格字符串第29和30个的位置）
               $new_1 = substr($newstr,0,29);
               $new_2 = substr($newstr,30);
               $replace_content = $new_1.' '.$new_2;
           }
        }else{
            $replace_content = $newstr;
        }
        if(!empty($replace_content)){
            $new_content[] = $replace_content.$last;
        }
    }
    $new_content = trim(implode("\n",$new_content));

    //生成受体文件
    file_put_contents($output_file_path.$receptor.'.pdbqt',$new_content);
    ...
    exec("cd $output_file_path ; /root/anaconda2/envs/pliptool/bin/obabel -ipdb '{$v['receptor']}' -opdb -O receptor_for_xscore.pdb -h;xscore -fixpdb 'receptor_for_xscore.pdb' receptor_for_xscore.pdb ; ");
    //使用openbabel转格式
    ...
    foreach ($files as $key => $file) {
        if (preg_match("/" . patt_trans($ligand_pdbqt) . "_out_ligand_(.*?).pdbqt/", $file)) {
            ...
            //用obabel更换个时候再加氢
            exec("cd $output_file_path ; /root/anaconda2/envs/pliptool/bin/obabel -ipdbqt '$file' -opdb -O $file_prefix.pdb;/root/anaconda2/envs/pliptool/bin/obabel -ipdb '$file_prefix.pdb' -omol2 -O '$mol_name' -h");
            //追加写入 ligand.mol
            ...
            if ($is_xscore === "true" || $is_xscore === true) {
                ...
                $xlog = [];
                exec("cd $output_file_path ; xscore -fixmol2 '$mol_name' ligand_for_xscore.mol2 ; xscore -score receptor_for_xscore.pdb ligand_for_xscore.mol2 ; ", $xlog);
                usleep(100000);
