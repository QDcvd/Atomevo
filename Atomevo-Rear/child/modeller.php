...
if (is_file($out_path . 'parameter/configure.json')) {
    $excel->jsonToCsv($out_path . 'parameter/configure.json', $out_path . 'result/configure.csv'); //保存配置信息为表格
}
...
//移出执行py文件
exec('cd '.$task['out_path'].'result ; mv *.py ../input_file/ ;');
//压缩打包所有生成文件
if($task['down_name']){ //改名
    exec('cd '.$task['out_path'].'result ; /usr/bin/zip -D -r \('.$task['down_name'].'\)-allFile.zip * ;');
    $file = $task['out_path'] . '('.$task['down_name'].')-allFile.zip';
}else{
    exec('cd '.$task['out_path'].'result ; /usr/bin/zip -D -r ./allFile.zip *');
...
$body = '<h1>Magical_modeller任务完成：'.$tasks_id.'</h1>';
