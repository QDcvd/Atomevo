<template xmlns="http://www.w3.org/1999/html">
  <div>
    <div class="mag-step">
      <div style="font-weight: bold;margin: 0.5rem">RGB
    ...
    <el-row :gutter="20">
      <el-col :lg="24" :xl="12">
        <div class="mag-step-left">
          ...
          <UpLoadFile FormatType=".tif,.csv" UseType="rgb" @GetIDList="GetFileID" :MaxSize="20"></UpLoadFile>
        </div>
        <div class="mag-step-left">
          ...
          <div style="margin-top: 1rem">
            <el-button type="primary" size="small" @click="RunRgb" :disabled="StepIndex==1?false:true">
              {{$t('btn.run')}}
            </el-button>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      //任务id
      ...mapState({
        TaksID:state => state.TaskIDList.RGB
      }),
    },
    watch: {
    ...
    methods: {
      ...
      RunRgb() {
        ...
        this.$api.GainRgb(Params).then(res => {
          // console.log(res)
          this.$notify({
