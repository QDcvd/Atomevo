<template>
  <div>
      ...
      <el-col :lg="24" :xl="12">
        ...
        <div class="mag-step-left">
          <el-radio-group v-model="choice" size="medium">
            <el-radio-button label="1">Exp4overlapDonor</el-radio-button>
            <el-radio-button label="2">Exp4gRNA</el-radio-button>
          </el-radio-group>
          ...
          <div style="margin-top: 1rem">
            <el-button
              type="primary"
              size="small"
              :disabled="StepIndex == 1 || choice == 2 ? false : true"
              @click="GainExp4cas9"
            >
              {{ $t("btn.run") }}
            </el-button>
    ...
    <el-drawer
      :with-header="false"
      :visible.sync="donorDrawer"
      direction="rtl"
      size="70%"
      :destroy-on-close="true"
      :before-close="handleDonorClose"
    >
      <excel-edit ModeName="Exp4overlapDonor" @HandleExcel="GetExcelData" />
    </el-drawer>
    <el-drawer
      :with-header="false"
      :visible.sync="rnaDrawer"
      direction="rtl"
      size="70%"
      :destroy-on-close="true"
      :before-close="handleRnaClose"
    >
      <excel-edit ModeName="Exp4gRNA" @HandleExcel="GetExcelData" />
    </el-drawer>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleOpenDrawer(type) {
      this.choice = type;
      if (type === 1) {
        this.rnaDrawer = false;
        this.donorDrawer = true;
      } else {
        this.donorDrawer = false;
        this.rnaDrawer = true;
      }
    },
    //关闭模态框
